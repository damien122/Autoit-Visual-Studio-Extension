...
const defaultSigs = Object.assign(
    {},
    mainFunctions,
    udfs,
    require("./signatures/udf_word"),
    require("./signatures/udf_winnet"),
    require("./signatures/udf_array")
)
...
module.exports = languages.registerSignatureHelpProvider(
    { language: "autoit", scheme: "file" },
    {
        provideSignatureHelp(document, position) {
            ...
            let thisSignature = new SignatureInformation(
                foundSig.label,
...
function getIncludes(doc) {
    ...
    while ((pattern = LIBRARY_INCLUDE_PATTERN.exec(text))) {
        ...
        if (DEFAULT_UDFS.indexOf(filename) == -1) {
            ...
            if (fullPath) {
                let newData = getIncludeData(fullPath, doc)
...
function getIncludeData(fileName, doc) {
    ...
    var functions = {}
    var filePath = getIncludePath(fileName, doc)
